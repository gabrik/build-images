...
docker exec build-z bash -c "eval \$(opam env) && OPAMSOLVERTIMEOUT=3600 opam pin add zenoh-tx-inet https://github.com/atolab/zenoh.git#0.3.0 -y"
docker exec build-z bash -c "eval \$(opam env) && OPAMSOLVERTIMEOUT=3600 opam pin add zenoh-router https://github.com/atolab/zenoh.git#0.3.0 -y"
...
chmod 0600 key
scp -o StrictHostKeyChecking=no -i ./key ../zenoh_${VERSION}-1_arm64.deb $USER@$SERVER:$DEPLOYDIR/fos/deb/bionic/arm64/
rm key
set -x