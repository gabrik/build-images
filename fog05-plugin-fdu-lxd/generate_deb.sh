...
# building deb file
docker exec build bash -c "cd /root/ && git clone https://github.com/gabrik/plugin-fdu-lxd"
...
docker exec build bash -c "export DEBEMAIL=\"info@adlink-labs.tech\" && export DEBFULLNAME=\"ADLINK Technology Inc\" && cd /root/build/fog05-plugin-fdu-lxd-0.1 && dh_make -f ../fog05-plugin-fdu-lxd-0.1.tar.gz -s -y"
...
docker cp templates/changelog build:/root/build/fog05-plugin-fdu-lxd-0.1/debian/changelog
docker cp templates/postinst build:/root/build/fog05-plugin-fdu-lxd-0.1/debian/postinst
docker cp templates/postrm build:/root/build/fog05-plugin-fdu-lxd-0.1/debian/postrm
docker cp templates/control build:/root/build/fog05-plugin-fdu-lxd-0.1/debian/control
docker cp templates/copyright build:/root/build/fog05-plugin-fdu-lxd-0.1/debian/copyright
...
docker cp build:/root/build/fog05-plugin-fdu-lxd_0.1-1_amd64.deb ../fog05-plugin-fdu-lxd_0.1-1_amd64_debian_buster.deb
