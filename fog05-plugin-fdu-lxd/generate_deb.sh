...
docker exec build-lxd apt install build-essential devscripts lintian dh-make git wget jq python3 python3-dev python3-pip unzip cmake sudo libxml2-dev libxslt-dev -y
...
docker cp build-lxd:/root/build/fog05-plugin-fdu-lxd_${VERSION}-1_arm64.deb ../fog05-plugin-fdu-lxd_${VERSION}-1_arm64.deb
...
if ["$UPLOAD" = true ]; then
    ...
    chmod 0600 key
    scp -o StrictHostKeyChecking=no -i ./key ../fog05-plugin-fdu-lxd_${VERSION}-1_arm64.deb $USER@$SERVER:$DEPLOYDIR/fos/deb/bionic/arm64/fog05-plugin-fdu-lxd-1_arm64.deb
    set -x
fi