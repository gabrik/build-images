...
if [ "$UPLOAD" = true ]; then
    ...
    chmod 0600 key
    scp -o StrictHostKeyChecking=no -i ./key ../fog05-plugin-fdu-native_${VERSION}-1_arm64.deb $USER@$SERVER:$DEPLOYDIR/fos/deb/bionic/arm64/fog05-plugin-fdu-native-1_arm64.deb
    rm key
    set -x
fi