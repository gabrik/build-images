...
docker cp build-agent:/root/build/fog05_${VERSION}-1_arm64.deb ../fog05_${VERSION}-1_arm64.deb
...
if [ "$UPLOAD" = true ]; then
    ...
    chmod 0600 key
    scp -o StrictHostKeyChecking=no -i ./key ../fog05_${VERSION}-1_arm64.deb $USER@$SERVER:$DEPLOYDIR/fos/deb/bionic/arm64/fog05-1_arm64.deb
    rm key
    set -x
fi
