...
docker exec build-force bash -c "cd /root/ && wget https://dl.google.com/go/go1.13.8.linux-amd64.tar.gz && tar -C /usr/local -xzf  go1.13.8.linux-amd64.tar.gz"
...
sg docker -c "docker build ./ -f ./Dockerfile.amd64 -t fog05/force --no-cache" --oom-kill-disable