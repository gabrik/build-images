...
lxc launch images:${IMAGE} build-ctd
sleep 2
# install deps
...
lxc exec build-ctd -- bash -c "export PATH=\$PATH:/usr/local/go/bin && cd /root/plugin-fdu-containerd && make && ldd plugin"
lxc exec build-ctd -- bash -c "mkdir /root/build && cd /root && cp -r plugin-fdu-containerd build/fog05-plugin-fdu-containerd-${VERSION} && cd build/fog05-plugin-fdu-containerd-${VERSION} && rm -rf .git && make clean && cd .. && tar -czvf fog05-plugin-fdu-containerd-${VERSION}.tar.gz fog05-plugin-fdu-containerd-${VERSION}"
lxc exec build-ctd -- bash -c "export DEBEMAIL=\"info@adlink-labs.tech\" && export DEBFULLNAME=\"ATO Labs\" && cd /root/build/fog05-plugin-fdu-containerd-${VERSION} && dh_make -f ../fog05-plugin-fdu-containerd-${VERSION}.tar.gz -s -y"
...
lxc file push templates/changelog build-ctd/root/build/fog05-plugin-fdu-containerd-${VERSION}/debian/changelog
lxc file push templates/postinst build-ctd/root/build/fog05-plugin-fdu-containerd-${VERSION}/debian/postinst
lxc file push templates/control build-ctd/root/build/fog05-plugin-fdu-containerd-${VERSION}/debian/control
