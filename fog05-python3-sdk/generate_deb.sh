...
docker exec build bash -c "cd /root/ && git clone https://github.com/gabrik/sdk-python fog05-sdk-0.1"
# build package
docker exec build bash -c "cd /root/fog05-sdk-0.1 && make"
#build deb
docker exec build bash -c "cd /root && mkdir build && tar -czvf build/fog05-sdk-0.1.tar.gz fog05-sdk-0.1"
docker exec build bash -c "cd /root/build && py2dsc fog05-sdk-0.1.tar.gz"
docker exec build bash -c "cd /root/build/deb_dist/fog05-sdk-0.1.0 && dpkg-buildpackage -rfakeroot -uc -us"
docker exec build bash -c "cd /root/build/deb_dist/ && dpkg -I python3-fog05-sdk_0.1.0-1_all.deb"
docker cp build:/root/build/deb_dist/python3-fog05-sdk_0.1.0-1_all.deb ../python3-fog05-sdk_0.1.0-1_all_debian_buster.deb
